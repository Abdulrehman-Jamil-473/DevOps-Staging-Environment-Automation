...
Checkout — pulls the latest code from the master branch
Test — sanity-checks that Ansible and Docker are available on the agent
Docker Build — builds the staging-app:latest image from the Dockerfile
Deploy — runs ansible-playbook -i inventory start.yml to launch/refresh the container
pipeline {
    agent any
    stages {
        stage('Checkout') {
